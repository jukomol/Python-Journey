...
progressbar=ttk.Progressbar(window, orient=HORIZONTAL,length=180,mode='determinate')

def step():
    progressbar['value']=100
    Label(window, text="Downloaded",width=12).grid(row=5,column=1,padx=5,pady=5)
    #progressbar.start(5)
    '''
    for i in range(5):
...
progressbar.grid(row=4,column=0,pady=5,columnspan=4)
